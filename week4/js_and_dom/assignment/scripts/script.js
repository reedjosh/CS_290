...
function setHighlightGen(coords){
    return function setHighlight(){
        cellAt(coords).style.backgroundColor = "yellow";
        };
    }
...
function makeButtons(){
    ...
    for (var key in lst){
        ...
        bText = document.createTextNode(key);
        btn.appendChild(bText);
        btn.onclick = modifyCoords(lst[key], currentCoord);
        document.body.appendChild(btn);
        }
    btn = document.createElement("button");
    bText = document.createTextNode("Mark Cell");
    btn.onclick = modifyCoords(lst[key], currentCoord);
    btn.appendChild(bText);
    btn.onclick = setHighlightGen(currentCoord);
    document.body.appendChild(btn);
